...
public class AsyncCalcSplitRule {
    ...
    /**
     * An Async implementation of {@link RemoteCalcCallFinder} which finds uses of
     * AsyncScalarFunction.
     */
    ...
    public static class AsyncCalcSplitNestedRule extends RemoteCalcSplitRuleBase<Void> {
        ...
        @Override
        public Tuple3<Option<RexNode>, Option<RexNode>, Seq<RexNode>> split(
                RexProgram program, ScalarFunctionSplitter splitter) {
            return new Tuple3<>(
    ...
    /**
     * This assumes that the nested rule has been run before this since it just splits async calls
     * so that there's one per calc.
     */
