...
class ExpressionReducer(
  ...
  override def reduce(
    ...
    val asyncUDFExprs = new ListBuffer[RexNode]()

    val literals = skipAndValidateExprs(rexBuilder, constExprs, asyncUDFExprs)
    ...
    while (i < constExprs.size()) {
      ...
      if (asyncUDFExprs.exists(_ eq unreduced)) {
  ...
  private def skipAndValidateExprs(
      rexBuilder: RexBuilder,
      constExprs: java.util.List[RexNode],
      asyncUDFExprs: ListBuffer[RexNode]): List[RexNode] = {
    constExprs.asScala
